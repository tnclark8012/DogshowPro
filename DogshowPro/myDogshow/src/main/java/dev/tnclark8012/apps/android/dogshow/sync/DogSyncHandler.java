...
public class DogSyncHandler {
	private static final String TAG = makeLogTag(DogSyncHandler.class);
	Context mContext;
	...
	public void sync(ContentResolver resolver, long lastSync, int flags) {
		...
		if (!overwritting) {
			Cursor currentDogsIdCursor = resolver
					.query(Dogs.CONTENT_URI, new String[] { Dogs.DOG_ID },
							null, null, Dogs.DEFAULT_SORT);
			...
			Cursor locallyUpdated = resolver.query(Dogs.CONTENT_URI,
					Query.DogSyncQuery.PROJECTION, selection, selectionArgs,
					Dogs.DEFAULT_SORT);
			...
			while (locallyUpdated.moveToNext()) {
				dog = new Dog();
				dog.breedString = locallyUpdated
						.getString(Query.DogSyncQuery.DOG_BREED);
				dog.callName = locallyUpdated
						.getString(Query.DogSyncQuery.DOG_CALL_NAME);
				dog.champion = locallyUpdated
						.getInt(Query.DogSyncQuery.DOG_IS_CHAMPION);
				dog.identifier = locallyUpdated
						.getString(Query.DogSyncQuery.DOG_ID);
				dog.majors = locallyUpdated
						.getInt(Query.DogSyncQuery.DOG_MAJORS);
				dog.modifiedTimeUtc = locallyUpdated
						.getLong(Query.DogSyncQuery.DOG_UPDATED);
				dog.points = locallyUpdated
						.getInt(Query.DogSyncQuery.DOG_POINTS);
				dog.sex = locallyUpdated.getInt(Query.DogSyncQuery.DOG_SEX);
				dog.showing = locallyUpdated
						.getInt(Query.DogSyncQuery.DOG_IS_SHOWING);
				dog.showingSweepstakes = locallyUpdated
						.getInt(Query.DogSyncQuery.DOG_IS_SHOWING_SWEEPSTAKES);
				dog.veteran = locallyUpdated
						.getInt(Query.DogSyncQuery.DOG_IS_VETERAN);
				locallyUpdatedDogs[i++] = dog;
				// Create json entities and post to server with current sync and
				// last sync time.
				// Response from server should contain dogs which have changed
				// since the last sync.
			}
		}

		// Push my dogs and get back those that need updating locally
		DogSyncResponse[] actionable = mAccessor.syncDogs(
				AccountUtils.getUserIdentifier(mContext),
				Prefs.getCurrentTeamIdentifier(mContext),
				SyncHelper.getLastSync(mContext), locallyUpdatedDogs,
				currentDogIds);
				...
				case DogSyncResponse.ACTION_UPDATE:
					builder = ContentProviderOperation
							.newUpdate(DogshowContract
									.addCallerIsSyncAdapterParameter(Dogs.CONTENT_URI));
					builder.withSelection(Dogs.DOG_ID + "=?",
							new String[] { response.dog.identifier });
					builder.withValues(getDogContentValues(response.dog));
					batch.add(builder.build());
					break;
	...
	public ContentValues getDogContentValues(Dog dog) {
		...
		values.put(Dogs.DOG_ID, dog.identifier);
		values.put(Dogs.DOG_IS_SHOWING, dog.showing);
		...
		values.put(Dogs.DOG_OWNER_ID, -1);// FIXME ALPHA dog owner Id should
											// just be the creator id like the
											// server
		values.put(Dogs.DOG_SEX, dog.sex);
		values.put(Dogs.DOG_IS_VETERAN, dog.veteran);
		values.put(Dogs.DOG_IS_CHAMPION, dog.champion);
