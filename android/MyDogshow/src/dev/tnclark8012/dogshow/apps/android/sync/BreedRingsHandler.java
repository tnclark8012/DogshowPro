...
public class BreedRingsHandler extends JsonHandler {
	...
	public ArrayList<ContentProviderOperation> parse(String showId,
			...
			long blockStartMillis, int count, String breedName, int dogCount,
			int bitchCount, int specialDog, int specialBitch, int countAhead) {
	...
	public ArrayList<ContentProviderOperation> parse(String jsonStr) {
		...
		Log.e("BreedRingsResponse", jsonStr);
		BreedRingsResponse response = new Gson().fromJson(jsonStr,
				BreedRingsResponse.class);
		int numRings = 0;
		...
		if (numRings > 0) {
			...
			for (BreedRing ring : response.breedRings) {
				// Insert rings info
				Log.i(TAG, "new add " + ring.bitchCount);
				batch.add(ContentProviderOperation
						.newInsert(
								DogshowContract
										.addCallerIsSyncAdapterParameter(BreedRings.CONTENT_URI))
						...
						.withValue(BreedRings.RING_BLOCK_START,
						...
						.withValue(BreedRings.RING_SPECIAL_BITCH_COUNT,
								ring.specialBitchCount)
