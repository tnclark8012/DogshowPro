...
public class AzureJuniorsRingsHandler extends JsonHandler {
	boolean clearExisting = false;
	boolean hasCleared = false;

	protected AzureJuniorsRingsHandler(Context context, boolean clearExisting) {
		super(context);
		this.clearExisting = clearExisting;
	}

	private static final String TAG = AzureJuniorsRingsHandler.class.getSimpleName();

	public ArrayList<ContentProviderOperation> parse(String jsonStr) {
		final ArrayList<ContentProviderOperation> batch = new ArrayList<ContentProviderOperation>();
		Gson gson = new GsonBuilder().setFieldNamingPolicy(FieldNamingPolicy.UPPER_CAMEL_CASE).create();
		JuniorsRing[] juniorsRings = gson.fromJson(jsonStr, JuniorsRing[].class);
		int numRings = 0;
