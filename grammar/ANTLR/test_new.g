...
inner_timeblock returns [JsonArray array]
	...
	:	 (judge_name|nonGroupRing=non_group_ring{nonGroupRing.add("CountAhead",new JsonPrimitive(countAhead));countAhead+=nonGroupRing.get("Count").getAsInt();array.add(nonGroupRing);})+

non_group_ring returns [JsonObject json]
@init(json = new JsonObject(); JsonObject ring;}:
	INT{json.addProperty("Count", parseIntSafely($INT.text,0));} (mRingWithBreed=ring_with_breed{mergeJson(mRingWithBreed,ring);})|(mRingWithoutBreed=ring_without_breed{mergeJson(mRingWithoutBreed,ring);});


rally_walkthrough [JsonObject json]
	@init{json = new JsonObject();}:
		RALLY_CLASS;//COUL - RALLY_CLASS: Rally Excellent Walkthrough 

ring_with_breed [JsonObject json]
@init(json = new JsonObject(); JsonObject ring;}:
	breedName=breed_name{mergeJson(json,breedName);} (suffix=special_suffix{mergeJson(json,suffix);}|((BREED_COUNT{int counted = addBreedCountToJson(json, $BREED_COUNT.text);assert (counted==total);})?));
	
special_suffix [JsonObject json]
@init{json = new JsonObject();}:
	SPECIAL_SUFFIX{json.addProperty("SpecialSuffix",$SPECIAL_SUFFIX.text);};

breed_name [JsonObject json]
...
ring_without_breed [JsonObject json]
@init(json = new JsonObject(); JsonObject ring;}:(mJuniorRing=junior_ring{mergeJson(mJuniorRing,json);})|
...
empty_breed_ring returns [JsonObject json]
	(BREED_COUNT{int counted = addBreedCountToJson(json, $BREED_COUNT.text);assert (counted==total);});
...
non_conformation_ring returns [JsonObject json]
 NON_CONFORMATION_CLASS_NAME{json.addProperty("Class",$NON_CONFORMATION_CLASS_NAME.text);} (NON_CONFORMATION_SECOND_LINE{json.addProperty("Entrants",$NON_CONFORMATION_SECOND_LINE.text);}|entrant=INT{json.addProperty("Entrants",$entrant.text);}) (NON_CONF_SECOND_LINE_COMMENT)*);
